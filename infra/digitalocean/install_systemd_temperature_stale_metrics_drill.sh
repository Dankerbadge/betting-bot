...
SERVICE_NAME="betbot-temperature-stale-metrics-drill"
TIMER_NAME="${SERVICE_NAME}.timer"
ENV_FILE="/etc/betbot/temperature-shadow.env"
SERVICE_FILE="/etc/systemd/system/${SERVICE_NAME}.service"
TIMER_FILE="/etc/systemd/system/${TIMER_NAME}"
RUN_SCRIPT="$REPO_DIR/infra/digitalocean/run_temperature_stale_metrics_drill.sh"
INTERVAL="${1:-6h}"
ON_BOOT_SEC="${2:-25m}"
...
[Unit]
Description=BetBot stale-metrics adaptive guardrail drill
...
[Unit]
Description=Run BetBot stale-metrics drill every $INTERVAL

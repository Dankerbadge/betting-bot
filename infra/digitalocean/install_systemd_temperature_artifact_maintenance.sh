...
SERVICE_NAME="betbot-temperature-artifact-maintenance"
TIMER_NAME="${SERVICE_NAME}.timer"
ENV_FILE="/etc/betbot/temperature-shadow.env"
SERVICE_FILE="/etc/systemd/system/${SERVICE_NAME}.service"
TIMER_FILE="/etc/systemd/system/${TIMER_NAME}"
RUN_SCRIPT="$REPO_DIR/infra/digitalocean/run_temperature_artifact_maintenance.sh"
INTERVAL="${1:-30m}"
...
[Unit]
Description=BetBot artifact maintenance (retention prune for high-churn outputs)
...
[Unit]
Description=Run BetBot artifact maintenance every $INTERVAL

...
2. Do not reuse any legacy external project URL, project ref, API key, database, or Vercel project.
3. Keep credentials namespaced to `OPSBOT_*` only.
4. Keep this dashboard read-only (no insert/update/delete code paths).

## Naming Convention

Use non-forbidden-target names such as:
...
1. Create a new Supabase project (do not clone legacy external project).

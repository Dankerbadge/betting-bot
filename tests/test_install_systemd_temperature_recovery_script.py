...
def _prepare_script_bundle(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    src_script = root / "infra" / "digitalocean" / "install_systemd_temperature_recovery.sh"
    ...
    run_script = script_dir / "run_temperature_pipeline_recovery.sh"
    ...
    service_file = tmp_path / "systemd" / "betbot-temperature-recovery.service"
    timer_file = tmp_path / "systemd" / "betbot-temperature-recovery.timer"
...
def test_installer_warns_with_remediation_when_any_strict_gate_disabled_or_missing(tmp_path: Path) -> None:

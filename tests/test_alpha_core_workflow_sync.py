...
ALPHA_CORE_WORKFLOW_PATH = ROOT / ".github" / "workflows" / "alpha-core.yml"

ALPHA_CORE_REQUIRED_TRIGGER_PATHS = {
    "betbot/**",
    "Makefile",
    ".github/workflows/alpha-core.yml",
}


def _read_required_text(path: Path) -> str:
    assert path.exists(), f"Expected file is missing: {path}. Create it or fix the path in this test."
...
def _parse_makefile_test_list(makefile_text: str, variable_name: str) -> list[str]:
    ...
    raise AssertionError(
        f"Could not find a {variable_name} := ... assignment in Makefile. "
        f"Add {variable_name} with test paths and keep it in sync with the workflow triggers."
    )


def _parse_alpha_core_tests_from_makefile(makefile_text: str) -> list[str]:
    return _parse_makefile_test_list(makefile_text, "ALPHA_CORE_TESTS")
...
def _find_top_level_key(lines: list[str], key: str) -> tuple[int, int]:
    ...
    for index, raw_line in enumerate(lines):
        ...
        if _line_indent(raw_line) == 0 and stripped == target:
            return index, 0
    raise AssertionError(
        f"Workflow is missing top-level '{target}' section. Add it to {ALPHA_CORE_WORKFLOW_PATH}."
    )
...
def _parse_yaml_dash_list(
    ...
    *,
    ...
    assert values, f"No entries found under '{context}'. Add path filters there."
    return values
...
def test_alpha_core_workflow_invokes_make_target() -> None:
    workflow_text = _read_required_text(ALPHA_CORE_WORKFLOW_PATH)

    assert _workflow_contains_run_command(
        workflow_text, "make test-alpha-core"
    ), "Expected .github/workflows/alpha-core.yml to include a run step: 'make test-alpha-core'."


def test_alpha_core_workflow_verifies_clean_tree() -> None:
    workflow_text = _read_required_text(ALPHA_CORE_WORKFLOW_PATH)

    assert _workflow_contains_run_command(
        workflow_text, "git diff --exit-code"
    ), "Expected .github/workflows/alpha-core.yml to include a clean-tree guard: 'git diff --exit-code'."


def test_alpha_core_workflow_trigger_paths_include_core_entries() -> None:
    workflow_text = _read_required_text(ALPHA_CORE_WORKFLOW_PATH)
    ...
    for trigger_name, trigger_paths in (
        ...
        missing = sorted(ALPHA_CORE_REQUIRED_TRIGGER_PATHS - trigger_paths)
        assert (
            not missing
        ), f"Missing required entries in on.{trigger_name}.paths: {missing}. Update .github/workflows/alpha-core.yml."


def test_alpha_core_makefile_tests_are_covered_by_workflow_path_filters() -> None:
    makefile_text = _read_required_text(MAKEFILE_PATH)
    workflow_text = _read_required_text(ALPHA_CORE_WORKFLOW_PATH)

    alpha_core_tests = _parse_alpha_core_tests_from_makefile(makefile_text)
    push_paths = set(_extract_workflow_paths(workflow_text, "push"))
    pull_request_paths = set(_extract_workflow_paths(workflow_text, "pull_request"))

    missing_in_push = [test_path for test_path in alpha_core_tests if test_path not in push_paths]
    missing_in_pull_request = [
        test_path for test_path in alpha_core_tests if test_path not in pull_request_paths
    ]

    assert (
        not missing_in_push
    ), f"These ALPHA_CORE_TESTS entries are missing from on.push.paths: {missing_in_push}. Sync .github/workflows/alpha-core.yml with Makefile."
    assert (
        not missing_in_pull_request
    ), f"These ALPHA_CORE_TESTS entries are missing from on.pull_request.paths: {missing_in_pull_request}. Sync .github/workflows/alpha-core.yml with Makefile."
